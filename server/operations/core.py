...
DEFAULT_PATTERN = re.compile(
    (
        ...
        "(?:[λΛ]εωφόρου? )|(?:(?:της )?Λεωφ. )|[νΝ]ομού |νομούς.* και |νομών.* και |πλατείας? |σταθμό |"
        "νήσου? |λίμνης? |Π.Ε. |Δ.Ε. |Ε.Ο. |Π.Ε.Ο. |Τ.Κ. |"
        "οδό |οδού |οδών.*και |δασάκι |φαράγγι (?:του|της|των )?|"
        "περιοχής? |(?:[Δδ]ήμο του|της )?|(?:[Δδ]ήμου?) |οικισμ[ό|ού] |"
...
def capital_words(text: str) -> str:
    """
    ...
    for sentence in sentences:
        ...
        for word in sentence.split()[1:]:
            if len(word) < 2:
                continue
            unicode = ord(word[0])
            # 902: Ά, 904-911: Έ, Ή, ..., Ώ, 913-937: Α, Β, Γ, ..., Ω
            if (
                unicode == 902
                or (unicode >= 904 and unicode <= 911)
                or (unicode >= 913 and unicode <= 937)
            ):
