...
def get_tweets_with_filter(
    department: str,
    order_by: str = None,
    order: str = "ASC",
    limit: int = None,
    location_only: bool = True,
) -> Any:
    departmentTable = str2department(department=department)
...
def save_tweets(tweet: Union[dict, List]) -> None:
    ...
    for tweet in tweets:
        ...
        try:
            ...
            print(
                f"The {new_tweet.__tablename__} tweet with id {new_tweet.id} \
                  already exists in DB!"
            )
    return None


def create_new_tweet(tweet: dict) -> Union[PoliceTweets, PyrosvestikiTweets]:
    """
    ...
    return departmentTable(
        id=id, text=text, created_at=created_at, longitude=longitude, latitude=latitude
    )
